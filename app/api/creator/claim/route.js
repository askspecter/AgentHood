...
async function skimAurnSplit({ chain, xUserId, wallet, token, weth, received }) {
  ...
  for (const a of assets) {
    ...
    // 70% landed → AURN's cut is 10/70; the creator keeps the other 60/70. The
    // whole 10/70 goes to the ops wallet.
    const slices = [{ to: opsTo, amount: (a.recv * 10n) / 70n, label: `ops:${a.key}` }];
