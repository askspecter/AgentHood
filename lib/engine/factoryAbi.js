...
async function fetchFactoryAbi(explorer, address, { timeoutMs = 12_000, attempts = 3 } = {}) {
  ...
  // Try each endpoint a few times with backoff. The block explorer (public
  // Blockscout) is the single dependency of the launch form, and it hiccups —
  // rate-limits, brief 5xx, slow responses — so one blip must not block a launch.
  ...
  // Every attempt failed. If we ever fetched this ABI successfully (even long
  // ago), serve that stale copy rather than blocking the launch — the factory's
  // ABI does not change, so a stale-but-real ABI is strictly better than an
  // outage. Only throw when we have never had it at all.
  if (hit) {
    console.warn(`Factory ABI refresh failed, serving stale copy. ${lastError || ""}`.trim());
    return hit.value;
  }
