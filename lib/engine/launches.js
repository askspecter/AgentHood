...
const { resolveBankrPool, v4Pricing } = require("./ponsV2");
 ...
 * A Bankr/Doppler token carries no on-chain `logo()` the way a pons launch does,
...
const BUILTIN_HIDDEN = [
  ...
  // The old $ESKA token, de-listed ahead of the @bankrbot relaunch. Hard-hidden
...
async function enrichLaunch(provider, chain, base) {
  ...
  try {
    ...
    if (launch.isPonsLaunch) {
      ...
      // Not a pons launch — e.g. a Bankr / Doppler token. Read its REAL supply
      ...
      try {
        const pool = await resolveBankrPool(provider, chain, base.token, chain.explorer);
        if (pool) {
      ...
      // Many tokens (Bankr/Doppler included) expose an on-chain logo() the same
