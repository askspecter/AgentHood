...
const { resolvePonsPool, v4Pricing, resolveLaunch, curvePricing } = require("./ponsV2");
...
/**
 * The token's icon from the block explorer's index (Blockscout), if it has one.
 * A Pons/Doppler token carries no on-chain `logo()` the way a pons launch does,
 * so this is how its image shows up in the feed instead of a placeholder orb.
 */
 ...
 */
/** Always hidden, on top of anything in HIDDEN_TOKENS (test launches, etc.). */
const BUILTIN_HIDDEN = [
  "0x2302a437536f1a668c69f7D0B1FbD089c630fe58", // $TEST - a test launch
  "0xF8f08e41F79A0564640EF68f5e1aAc144c133465", // $NYX - a test launch
  // The old legacy platform token, de-listed ahead of the @ponsdotfamily launch. Hard-hidden
  // so it can never appear in the feed or be pinned again - even from a cached
  // registry entry or a stale OFFICIAL_TOKEN. The new token gets a new address.
  "0x0eB9960654d3661D551a4536D7d425184eC81756",
  // $AURN - the platform token, removed from the site. Hard-hidden so it never
  // appears in the feed or gets pinned again, even if a stale OFFICIAL_TOKEN env
  // var or a cached registry entry still points at it.
  "0x15157A4380a357Fc307Ee39A4A476bE5ec8D2E74",
];
...
function hiddenTokenSet() {
  ...
  const add = (part) => {
    if (!part) return;
    try { out.add(getAddress(part).toLowerCase()); } catch { /* skip a bad address */ }
  };
  for (const a of BUILTIN_HIDDEN) add(a);
  const raw = process.env.HIDDEN_TOKENS;
  if (raw) for (const part of String(raw).split(/[\s,;]+/)) add(part);
  return out;
}
...
async function enrichLaunch(provider, chain, base) {
  ...
  try {
    ...
    if (launch.isPonsLaunch) {
      ...
      out.isToken0 = launch.isToken0 ?? null;
      out.deployer = out.deployer || launch.deployer || null;
      out.pairToken = launch.pairedToken || null;
      // Stock-paired = quoted in a tokenized stock/RWA, not WETH or native.
      const weth = (chain.pons?.weth || "").toLowerCase();
      out.stockPaired = Boolean(
        launch.pairedToken &&
        !/^0x0+$/.test(launch.pairedToken) &&
        launch.pairedToken.toLowerCase() !== weth
      );
      out.isPonsLaunch = true;
    } else {
      // Not a pons launch - e.g. a Pons / Doppler token. Read its REAL supply
      // (so the "Supply" figure and market cap aren't the pons 1B default), price
      // it on its Uniswap V4 pool when it has one, and pull its icon from the
      // explorer so it isn't a placeholder orb.
      out.isPonsLaunch = false;
      try {
        const [rawSupply, dec] = await Promise.all([
          meta.totalSupply().catch(() => null),
          meta.decimals().catch(() => 18),
        ]);
        out.decimals = Number(dec) || 18;
        if (rawSupply != null) out.supplyTokens = Number(formatUnits(rawSupply, out.decimals));
      } catch { /* keep whatever we have */ }
      try {
        // A pons v2 launch prices from the factory record (RPC only, no explorer):
        // the curve while it's bonding (phase 0), the v4 pool once graduated
        // (phase 2). This is the reliable path and covers most coins here.
        const lc = await resolveLaunch(provider, chain, base.token);
        if (lc) {
          out.pairToken = lc.pairToken ?? out.pairToken;
          out.poolFee = lc.poolFee ?? out.poolFee;
          if (lc.phase === 0) {
            const p = await curvePricing(provider, chain, lc);
            if (p.priceInWeth != null) out.priceInWeth = p.priceInWeth;
            if (p.marketCapWeth != null) out.marketCapWeth = p.marketCapWeth;
            if (p.graduationProgress != null) out.graduationProgress = p.graduationProgress;
            out.graduated = false;
          } else if (lc.phase === 2) {
            out.graduated = true;
            out.graduationProgress = 1;
            const p = await v4Pricing(provider, chain, lc);
            if (p.priceInWeth != null) out.priceInWeth = p.priceInWeth;
            if (p.marketCapWeth != null) out.marketCapWeth = p.marketCapWeth;
          }
        }
        // Not a v2 launch, or v2 pricing came up empty: fall back to discovering
        // any V4 pool that holds the token (via the explorer's log index).
        if (out.priceInWeth == null) {
          const pool = await resolvePonsPool(provider, chain, base.token, chain.explorer);
          if (pool) {
            out.pairToken = pool.pairToken;
            out.poolFee = pool.poolFee;
            const p = await v4Pricing(provider, chain, pool);
            if (p) {
              if (p.priceInWeth != null) out.priceInWeth = p.priceInWeth;
              if (p.marketCapWeth != null) out.marketCapWeth = p.marketCapWeth;
            }
      ...
      } catch { /* pricing is best-effort - the coin still shows without it */ }
      // Many tokens (Pons/Doppler included) expose an on-chain logo() the same
      // way a pons launch does - read it first, exactly like pons does, so the
      // coin's own image shows. Fall back to the explorer icon only if empty.
      try {
        const logoC = new Contract(base.token, ["function logo() view returns (string)"], provider);
        const lg = await logoC.logo().catch(() => null);
        if (lg && typeof lg === "string" && lg.trim()) out.logo = lg.trim();
      } catch { /* token has no logo() - fine */ }
      if (!out.logo) {
        try {
          const icon = await explorerTokenIcon(chain.explorer, base.token);
          if (icon) out.logo = icon;
