...
/* ── Graduated / external V4 pools ─────────────────────────────────────────
 * Some tokens trade on an ordinary Uniswap V4 pool on Robinhood Chain with their
 * OWN hook (not the pons hook) - a graduated v2 coin, or one deployed outside the
 * pons factory. resolveLaunch() can't see those, so we discover the pool straight
 * from the canonical V4 PoolManager's `Initialize` event - which carries the full
 * pool key (both currencies, fee, tickSpacing, hook) - and return a launch-shaped
 * object the same V4 quote/swap code path consumes. Works for any V4 pool holding
 * the token. */
...
const _ponsPoolCache = new Map(); // token(lower) -> { at, launch|null }
const PONS_POOL_TTL_MS = 5 * 60 * 1000;
...
/**
 * Resolve a token's tradeable Uniswap V4 pool (as launched through Pons), or
 ...
 */
async function resolvePonsPool(provider, chain, token, explorer) {
  const cfg = v2Config(chain);
  if (!cfg || !cfg.poolManager || !explorer || !token) return null;
  const key = String(token).toLowerCase();

  const cached = _ponsPoolCache.get(key);
  if (cached && Date.now() - cached.at < PONS_POOL_TTL_MS) return cached.launch;
  ...
  _ponsPoolCache.set(key, { at: Date.now(), launch });
  return launch;
}
...
function poolKeyFor(launch, chain) {
  ...
  return {
    currency0,
    currency1,
    fee: launch.poolFee, // pons: 0 (the hook charges the fee); Pons: the pool's own fee
    tickSpacing: launch.tickSpacing,
    // A pons launch uses the shared pons hook; a Pons pool carries its own hook
