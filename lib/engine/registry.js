 ...
 */
/**
 * The pinned official token. ESKA has none yet ($ESKA isn't launched), so there
 * is NO hardcoded default — nothing is pinned unless OFFICIAL_TOKEN is set. When
 * $ESKA exists, set OFFICIAL_TOKEN (and optionally OFFICIAL_SYMBOL/NAME/LOGO/X)
 * to pin it to the top of "launched here".
 */
function officialToken() {
  ...
  try {
    const socials = {};
    if (process.env.OFFICIAL_X) socials.x = `https://x.com/${process.env.OFFICIAL_X.replace(/^@/, "")}`;
    ...
    return {
      ...
      xUsername: process.env.OFFICIAL_X ? process.env.OFFICIAL_X.replace(/^@/, "") : null,
      socials,
    };
  } catch {
