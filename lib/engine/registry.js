 ...
 */
/**
 * No token is pinned by default. The official hero-card pin is purely opt-in via
 * OFFICIAL_TOKEN (e.g. a fork that wants its own token featured). With no
 * OFFICIAL_TOKEN set, the feed shows only coins launched here - nothing is pinned.
 * OFFICIAL_LOGO / OFFICIAL_X customize the pinned card when one is configured.
 */
const DEFAULT_OFFICIAL_TOKEN = "";
const DEFAULT_OFFICIAL_LOGO = "/aurn-logo.png";
const DEFAULT_OFFICIAL_X = "";

/** The pinned official token address: OFFICIAL_TOKEN if set, else none; "" drops it. */
function officialTokenAddress() {
  const env = process.env.OFFICIAL_TOKEN;
  const raw = env === undefined ? DEFAULT_OFFICIAL_TOKEN : env.trim();
  if (!raw) return null; // no OFFICIAL_TOKEN -> nothing pinned
