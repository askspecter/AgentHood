 ...
 */
/**
 * The pinned official token - the hero card, marked official with a gold verified
 * check. There is NO hardcoded default: nothing is pinned unless OFFICIAL_TOKEN is
 * set. Set OFFICIAL_TOKEN (plus optional OFFICIAL_SYMBOL / OFFICIAL_NAME /
 * OFFICIAL_X) to pin a token - e.g. the new coin after a relaunch.
 */
function officialToken() {
  // No env → nothing pinned. The pin is entirely opt-in.
  const raw = process.env.OFFICIAL_TOKEN;
  if (!raw) return null;
  try {
    const xh = (process.env.OFFICIAL_X || "").replace(/^@/, "");
    const socials = {};
