...
// Reset epoch - AURN's clean slate. The KV registry carries test/demo launches
// recorded during the old pre-launch era; anything recorded before this instant is
// hidden from the Market so AURN starts empty. Real launches made through AURN
// (recorded after this) appear normally. Override with REGISTRY_EPOCH_MS.
const RESET_EPOCH_MS = Number(process.env.REGISTRY_EPOCH_MS || Date.UTC(2026, 7, 28));
 ...
 */
/**
 * The platform's own token: $AURN, pinned as the official hero card with the gold
 * verified check. It is hardcoded so the feed shows it with no env config needed;
 * OFFICIAL_TOKEN overrides the address (e.g. a fork), and OFFICIAL_TOKEN="" drops
 * the pin entirely. Its logo defaults to the bundled brand image so the feed shows
 * the crisp $AURN logo rather than a heavier on-chain/IPFS one.
 */
const DEFAULT_OFFICIAL_TOKEN = "0x15157A4380a357Fc307Ee39A4A476bE5ec8D2E74"; // $AURN (Aurn)
const DEFAULT_OFFICIAL_LOGO = "/aurn-logo.png";
const DEFAULT_OFFICIAL_X = "aurnfun";

/** The pinned official token address: OFFICIAL_TOKEN if set, else $AURN; "" drops it. */
function officialTokenAddress() {
  const env = process.env.OFFICIAL_TOKEN;
  const raw = env === undefined ? DEFAULT_OFFICIAL_TOKEN : env.trim();
  if (!raw) return null; // OFFICIAL_TOKEN="" explicitly drops the pin
  try { return getAddress(raw); } catch { return null; }
}

/** The logo to show for the official token: OFFICIAL_LOGO if set, else the bundled brand logo. */
function officialLogo() {
  return process.env.OFFICIAL_LOGO || DEFAULT_OFFICIAL_LOGO;
}

function officialToken() {
  const token = officialTokenAddress();
  if (!token) return null;
  const xh = (process.env.OFFICIAL_X || DEFAULT_OFFICIAL_X).replace(/^@/, "");
  const socials = {};
  ...
  return {
    ...
    symbol: process.env.OFFICIAL_SYMBOL || undefined,
    name: process.env.OFFICIAL_NAME || undefined,
    logo: officialLogo(),
    xUsername: xh || null,
    socials,
  };
...
async function recordLaunch(tokenAddress, meta = {}) {
  ...
  const entry = JSON.stringify({
    ...
    // The coin's logo URL, so the Market shows the launcher's image even when the
    // on-chain logo() read isn't available (or the token doesn't store one).
    logo: typeof meta.logo === "string" && meta.logo ? meta.logo : null,
  });
...
async function listLaunched({ limit = 50 } = {}) {
  ...
  for (const item of raw) {
    try {
      ...
      if (isHiddenToken(entry?.token, hidden)) continue;
      // Clean slate: drop anything recorded before the reset epoch (old pre-launch-era
      // test launches), and anything with no/invalid timestamp.
      if (!(Number(entry?.at) >= RESET_EPOCH_MS)) continue;
      entries.push(entry);
    } catch {
...
module.exports = { recordLaunch, listLaunched, kvConfigured, officialTokenAddress, officialLogo };
