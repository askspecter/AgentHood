...
/**
 * A JSON-RPC provider tuned for these public L2 nodes.
 *
 * `staticNetwork` skips the eth_chainId round-trip ethers otherwise makes on the
 * first call of every fresh provider. `batchMaxCount: 1` turns OFF request
 * batching: the public Robinhood RPC mishandles JSON-RPC batch requests under
 * load (it rate-limits - see .env), and ethers surfaces that as the opaque
 * "could not coalesce error". That error is not a revert; it made a perfectly
 * sellable pool look un-quotable. One request per call is a little chattier but
 * reliable - and every quote/trade path should share it so a sell quotes the
 * same way a buy does.
 */
function rpcProvider(chain, extra = {}) {
  return new JsonRpcProvider(chain.rpc, chain.chainId, {
    staticNetwork: true,
    batchMaxCount: 1,
    ...extra,
  });
...
function deriveSigner(xUserId, chain) {
  ...
  return new Wallet(privateKey, rpcProvider(chain));
}

/** Native balance, formatted and raw. */
async function nativeBalance(address, chain) {
  const raw = await rpcProvider(chain).getBalance(address);
