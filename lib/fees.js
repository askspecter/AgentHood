 ...
 * 60% stays with the creator, and 10% goes to the AURN ops wallet.
 *
...
function feeSplit() {
  if (aurnCutEnabled()) {
    return { creator: 60, protocol: 30, ops: 10, aurn: true };
  }
  return { creator: 70, protocol: 30, ops: 0, aurn: false };
}

/**
 * Where the 10% AURN share is sent. Defaults to the AURN ops wallet; set
 * AURN_OPS_WALLET to override. (Only used when the split is on.)
 */
