 ...
 * Reacts to the pointer; auto-rotates on its own; pauses when off-screen.
 * Falls back to the CSS <HoloOrb/> where WebGL is unavailable.
 */
...
// pastel holographic cosine palette (lilac · sky · mint · peach · pink)
vec3 pal(float t){
...
void main(){
  vec2 uv = (gl_FragCoord.xy * 2.0 - u_res) / min(u_res.x, u_res.y); // -1..1, square
  vec2 p  = uv / 0.60;                 // sphere radius ~0.60 of the half-extent
  float r2 = dot(p, p);

  vec3  col = vec3(0.0);
  float alpha = 0.0;

  // spectral bloom that bleeds into the black
  ...
  if (r2 < 1.0){
    float z = sqrt(1.0 - r2);
    vec3  N = vec3(p, z);              // sphere surface normal
    vec3  V = vec3(0.0, 0.0, 1.0);     // view direction

    // spin the surface sampling → colour bands travel around the ball
    mat3 rot = rotY(u_time * 0.42 + u_mouse.x * 1.0) * rotX(-0.32 + u_mouse.y * 0.7);
    vec3 S = rot * N;
    float lon = atan(S.z, S.x);
    float lat = asin(clamp(S.y, -1.0, 1.0));

    float fres = pow(1.0 - max(dot(N, V), 0.0), 2.2);   // thin-film / rim term

    // holographic skin: longitude bands + latitude shimmer + fresnel interference
    float t = lon / TAU + 0.16 * sin(lat * 3.0 + u_time * 0.6) + fres * 0.55 + u_time * 0.05;
    vec3 base  = pal(t);
    vec3 base2 = pal(t * 2.0 + 0.33 + fres);            // second interference layer
    ...
    sphere += (hash(gl_FragCoord.xy + u_time) * 2.0 - 1.0) * 0.02;   // foil grain

    float edge = smoothstep(1.0, 0.982, r2);            // antialiased limb
...
export default function HoloSphere({ size = 300 }){
  ...
  useEffect(() => {
    ...
    function resize(){
      const w = Math.round(box * dpr), h = w
    ...
    const io = new IntersectionObserver(([en]) => {
      if (en.isIntersecting && !running && !reduce) { running = true; raf = requestAnimationFrame(frame) }
      else if (!en.isIntersecting) { running = false; cancelAnimationFrame(raf) }
    }, { threshold: 0.01 })
    io.observe(canvas)
    function onVis(){ if (document.hidden) { running = false; cancelAnimationFrame(raf) } else if (!running && !reduce) { running = true; raf = requestAnimationFrame(frame) } }
  ...
  if (failed) return <HoloOrb size={size} />
  ...
  return (
    <div className="relative grid place-items-center" style={{ width: box, height: box }}>
      ...
      <div className="absolute rounded-full pointer-events-none spin-slow"
        style={{ width: box * 0.82, height: box * 0.82, border: '1px solid rgba(255,255,255,0.06)' }} />
      <div className="absolute rounded-full pointer-events-none"
        style={{ width: box * 0.7, height: box * 0.7, border: '1px solid rgba(255,255,255,0.04)' }} />
      ...
      <div className="absolute pointer-events-none" style={{ width: box * 0.86, height: box * 0.86, animation: 'spinSlow 16s linear infinite' }}>
