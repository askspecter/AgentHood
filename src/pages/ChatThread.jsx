...
export default function ChatThread() {
  ...
  if (!charm) return <div className="text-center py-20 text-[var(--color-ink-soft)]">Charm not found.</div>
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col" style={{ minHeight: 'calc(100vh - 8rem)' }}>
      ...
      <div className="card p-4 flex items-center gap-3 mb-4">
        ...
        <Link to={`/c/${charm.id}`} className="btn btn-ghost text-xs !py-1.5">Trade</Link>
      </div>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto pr-1">
        {msgs.length === 0 && (
          <div className="card p-5 text-center">
            <CharmAvatar charm={charm} size={56} />
            <p className="mt-3 font-serif text-xl">{charm.name}</p>
            <p className="text-sm text-[var(--color-ink-soft)] mt-1 max-w-sm mx-auto">{charm.lore}</p>
            <div className="flex flex-wrap gap-2 justify-center mt-4">
              {starters.map((s) => (
                <button key={s} onClick={() => sendMessage(id, s)} className="chip hover:brightness-95">{s}</button>
              ))}
            </div>
          </div>
        )}
          ...
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div
              className={`max-w-[75%] px-4 py-2.5 text-sm leading-relaxed ${
                m.role === 'user'
                  ? 'bg-[var(--color-ink)] text-white rounded-2xl rounded-br-md ml-2'
                  : 'card !rounded-2xl !rounded-bl-md ml-2'
              }`}
            >
              {m.text}
            </div>
      ...
      <form onSubmit={submit} className="mt-4 flex gap-2 sticky bottom-20 md:bottom-4">
        <input
          ...
          className="flex-1 card px-4 py-3 text-sm outline-none focus:ring-2 ring-[var(--color-sky-deep)]"
        />
