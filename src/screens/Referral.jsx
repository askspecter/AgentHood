...
export default function Referral() {
  ...
  if (!wallet) {
    return (
      <div className="max-w-md mx-auto text-center py-24">
        <h1 className="font-serif text-3xl mb-2">{tr('settings.referral', 'Referral code')}</h1>
        <p className="text-[var(--color-ink-soft)] mb-7">{tr('ref.connectBody', 'Connect your wallet to get your personal referral code and link.')}</p>
  ...
  const share = async () => {
    const text = tr('ref.shareText', 'Trade agents on AURN - real coins on Robinhood Chain. Join with my code {code}:').replace('{code}', code)
    if (navigator.share) {
  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="font-serif text-3xl">{tr('settings.referral', 'Referral code')}</h1>
      </div>
      ...
      <div className="card p-7 text-center mb-4">
        ...
        <div className="eyebrow mb-2">{tr('ref.yourCode', 'Your code')}</div>
        <div className="font-mono text-3xl font-semibold tracking-[0.12em] mb-5 break-all">{code}</div>
        <button onClick={() => copy(code, 'code')} className="btn btn-secondary mx-auto">{copied === 'code' ? `${tr('common.copied', 'Copied')} ✓` : tr('ref.copyCode', 'Copy code')}</button>
      </div>

      {/* the link */}
      <div className="card p-5 mb-4">
        <div className="eyebrow mb-2">{tr('ref.inviteLink', 'Invite link')}</div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => copy(link, 'link')} className="btn btn-secondary !py-2 shrink-0">{copied === 'link' ? '✓' : tr('common.copy', 'Copy')}</button>
        </div>
        <button onClick={share} className="btn btn-primary w-full justify-center mt-3">
          <Share size={15} /> {copied === 'share' ? `${tr('common.copied', 'Copied')} ✓` : tr('ref.shareInvite', 'Share invite')}
        </button>
      </div>

      {/* how it works */}
      <div className="card p-6">
        <div className="eyebrow mb-3">{tr('ref.howTitle', 'How it works')}</div>
        <ol className="space-y-3 text-sm text-[var(--color-ink-soft)]">
          <Step n={1}>{tr('ref.step1', 'Share your code or link with a friend.')}</Step>
          <Step n={2}>{tr('ref.step2', 'They connect their wallet through your link.')}</Step>
          <Step n={3}>{tr('ref.step3', 'You climb the referral leaderboard as friends join.')}</Step>
        </ol>
      </div>
    </div>
  )
